 ...
 */
package com.mobaijun.oss.template;
...
public interface OssTemplate {
     ...
     * @throws software.amazon.awssdk.services.s3.model.BucketAlreadyExistsException     请求的存储桶名称不可用。存储桶名称空间由系统的所有用户共享。请选择其他名称然后重试。
     * @throws software.amazon.awssdk.services.s3.model.BucketAlreadyOwnedByYouException 您尝试创建的存储桶已经存在，并且您拥有它。 Amazon
     *                                                                                   S3在除北弗吉尼亚州以外的所有AWS地区均返回此错误。为了实现旧兼容性，如果您重新创建在北弗吉尼亚州已经拥有的现有存储桶， Amazon S3将返回200
     *                                                                                   OK并重置存储桶访问控制列表（ACL）
     * @throws software.amazon.awssdk.awscore.exception.AwsServiceException              SDK可能引发的所有异常的基类（不论是服务端异常还是客户端异常）。可用于所有场景下的异常捕获。
     * @throws software.amazon.awssdk.core.exception.SdkClientException                  如果发生任何客户端错误，例如与IO相关的异常，无法获取凭据等,会抛出此异常
     * @throws software.amazon.awssdk.services.s3.model.S3Exception                      所有服务端异常的基类。未知异常将作为此类型的实例抛出
     * @see <a href=
    ...
    /**
     * 上传一个本地文件到S3中的对象。对于非基于文件的上传，您可以使用 {@link #upload(software.amazon.awssdk.transfer.s3.model.UploadRequest)} 替代.
     * <p>
     * <b>使用示例:</b> <pre>
     * {@code
     * FileUpload upload =
     ...
     * </pre>
     *
     * @see #uploadFile(java.util.function.Consumer)
     * @see #upload(software.amazon.awssdk.transfer.s3.model.UploadRequest)
     */
     ...
     */
    default FileUpload uploadFile(Consumer<Builder> request) {
        return uploadFile(UploadFileRequest.builder().applyMutation(request).build());
    }

    /**
     * Upload the given {@link software.amazon.awssdk.core.async.AsyncRequestBody} to an object in S3. For file-based
     ...
     * upload.completionFuture().join();
     * }
     * </pre> See the static factory methods available in {@link software.amazon.awssdk.core.async.AsyncRequestBody} for
     * other use cases.
     *
     * @param uploadRequest the upload request, containing a {@link PutObjectRequest} and
     *                      {@link software.amazon.awssdk.core.async.AsyncRequestBody}
     * @return An {@link software.amazon.awssdk.transfer.s3.model.Upload} that can be used to track the ongoing transfer
     * @see #upload(Consumer)
     * @see #uploadFile(UploadFileRequest)
     */
